...
class StepDataCollector():
    ...
    def generate_tracking_statistics(self):
        ...
        mesh_one_global_ids = list(self.mesh_one.global_id_dictionary.keys())
        mesh_two_global_ids = list(self.mesh_two.global_id_dictionary.keys())
    ...
    def generate_death_statistics(self):
        ...
        for element in self.mesh_one.elements:
            if element.global_id not in list(self.mesh_two.global_id_dictionary.keys()):
                element_dyed = True
                if element.check_if_on_boundary():
                    ...
                    for frame_id in adjacent_element_ids:
                        ...
                        if adjacent_global_id not in list(self.mesh_two.global_id_dictionary.keys()):
                            element_dyed = False
                            break
                if element_dyed:
    ...
    def generate_centroid_statistics(self):
        ...
        for element in self.mesh_one.elements:
            if element.global_id in list(self.mesh_two.global_id_dictionary.keys()):
...
#         self.index_global_ids_from_largest_mappings()
    ...
    def extend_preliminary_mapping(self, next_frame_id, mapping_candidate):
        ...
        new_neighbour_ids = self.mesh_one.get_not_yet_mapped_shared_neighbour_ids( [next_frame_id],
                                                                                   list(self.preliminary_mappings.keys()) )
    ...
    def alternative_find_safe_mapping_candidate_for_single_cell(self, frame_id, relaxed_condition = False ):
        ...
        if ( frame_id not in self.preliminary_mappings ):
            ...
            mapping_candidates = self.mesh_two.get_not_yet_mapped_shared_neighbour_ids( images_of_already_mapped_neighbours,
                                                                                        list(self.preliminary_mappings.values()) )
        ...
        while not_all_neighbours_mapped:
            ...
            for node in network_one.nodes():
                if node not in preliminary_mapping and node not in self.preliminary_mappings:
                    ...
                    if mapping_candidate is not None and mapping_candidate not in list(preliminary_mapping.values()):
...
#                     import pdb; pdb.set_trace()
        ...
        connected_components_in_network_one = list( network_one.subgraph(c) for c in nx.connected_components(network_one) )
...
#        mappings_based_on_adjacency = self.get_mappings_by_adjacency(connected_component_one, connected_component_two)
        ...
        potential_mother_cells = self.mesh_one.get_not_yet_mapped_shared_neighbour_ids( list(bordering_cells_mapping.keys()) )
        ...
        if len(potential_mother_cells) == 0:
            ...
            new_potential_mother_cells = list(bordering_cells_mapping.keys())
            potential_daughter_cells = list(bordering_cells_mapping.values()) 
            # add the triangular cell
            # this `+' is a list concatenation
            potential_daughter_cells += self.mesh_two.get_not_yet_mapped_shared_neighbour_ids( list(bordering_cells_mapping.values()) ) 
...
#                 division_resolved = False
    ...
    def identify_division_event(self, potential_mother_cells, potential_daughter_cells, preliminary_mapping ):
        ...
        inverse_preliminary_mapping = { value: key for key, value in list(preliminary_mapping.items()) }
...
#         self.preliminary_mappings = copy.copy(self.largest_mappings[0])
        ...
        network_two = self.mesh_two.generate_network_of_unidentified_elements(list(self.preliminary_mappings.values()))
        connected_components_in_network_two = list( network_two.subgraph(c) for c in nx.connected_components(network_two) )
...
#         self.reindex_global_ids()
    ...
    def resolve_division_event_for_connected_component(self, connected_component):
        ...
        preliminary_adjacent_elements = list(set(unique_adjacent_elements).intersection( list(self.preliminary_mappings.values()) ))
        mcs_adjacent_elements = list(set(unique_adjacent_elements).intersection( list(self.largest_mappings[0].values()) ))
        
        # collect cells for connected_component_one
        inverse_preliminary_mapping = { value : key for key, value in list(self.preliminary_mappings.items()) }
        inverse_largest_mapping = { value : key for key, value in list(self.largest_mappings[0].items()) }
