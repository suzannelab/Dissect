...
class Segmentation2D(Segmentation):
    ...
    def skel_vertices(self):
        ...
        """
        df_skel_vertices = self.skeleton.critical_point[self.skeleton.critical_point.nfil>=3]
        
        self.vert_df = df_skel_vertices[['x','y']]
        self.vert_df = self.vert_df.reset_index(drop=True)
        df_test = pd.DataFrame(columns=['x_0','y_0'])
        df_test['x_0'] = self.vert_df['y']
        df_test['y_0'] = self.vert_df['x']
        self.vert_df = df_test
        vert_df_int = self.vert_df.astype('int32')
        
        return vert_df_int

    def create_edge_df(self, junc_df):

        columns = ['face', 'srce', 'trgt']
        init = np.zeros(((df_junc.shape[0])*2, 3))
        self.edge_df = pd.DataFrame(data=init, columns=columns)
        
        ind = 0
        for i in range (0, df_junc.shape[0]):
        
            self.edge_df['face'][ind] = df_junc['Cell 1'][i]
            self.edge_df['srce'][ind] = df_junc['vertices'][i][0]
            self.edge_df['trgt'][ind] = df_junc['vertices'][i][1]

            self.edge_df['face'][ind+1] = df_junc['Cell 2'][i]
            self.edge_df['srce'][ind+1] = df_junc['vertices'][i][1]
            self.edge_df['trgt'][ind+1] = df_junc['vertices'][i][0]
    
            ind = ind+2
        

        for i in np.arange(0, self.edge_df.shape[0], 2):
            segmentationi = np.zeros_like(mask)
            segmentationi[np.where(seg_nobound==edge_df.face[i])] = 1
            com = ndi.measurements.center_of_mass(segmentationi) #com = center of mass

            #calculate angle
            p1 = np.array([self.vert_df.x.iloc[int(self.edge_df.srce.iloc[i])], 
                           self.vert_df.y.iloc[int(self.edge_df.srce.iloc[i])]
                        ])

            p2 = np.array([self.vert_df.x.iloc[int(self.edge_df.trgt.iloc[i])], 
                           self.vert_df.y.iloc[int(self.edge_df.trgt.iloc[i])]
                        ])
            ...
            if np.sin(angle)<0:
                e = self.edge_df.srce.iloc[i]
                self.edge_df.srce.iloc[i] = self.edge_df.srce.iloc[i+1]
                self.edge_df.srce.iloc[i+1] = e
            
                f = self.edge_df.trgt.iloc[i]
                self.edge_df.trgt.iloc[i] = self.edge_df.trgt.iloc[i+1]
                self.edge_df.trgt.iloc[i+1] = f
    ...
    def find_vertex(self,
        ...
        for i in np.arange(len(kernel)):
            out = ndi.binary_hit_or_miss(mask, kernel[i])
        ...
        self.vert_df = df_vertices2.drop( ['Cell_1','Cell_2','Cell_3','Cell_4', 'Cell_5'], axis=1)
        return df_vertices2


    def find_cells(self, mask, dist):

        seg = segmentation(mask, size_auto_remove=False, min_area=2, max_area=10000, boundary_auto_remove=True)
        vert_df_int = self.skel_vertices()
